...
// local imports
import { OpenAiCostCalculator, pricingPerModel } from "../src/openai_cost_calculator";
...
describe('OpenAiCostCalculator.calculateCost', () => {
	...
	it('throws for unknown model name', async () => {
		...
		await Assert.rejects(
			() => OpenAiCostCalculator.calculateCost('nonexistent-model', usage),
	...
	it('strips date suffix from model name', async () => {
		...
		const usage = createResponseUsage({ input_tokens: 1_000_000, output_tokens: 1_000_000 });
		const cost = await OpenAiCostCalculator.calculateCost('gpt-4o-2024-08-06', usage);
	...
	it('selects <272K pricing for gpt-5.4 when under threshold', async () => {
		...
		const cost = await OpenAiCostCalculator.calculateCost('gpt-5.4', usage);
	...
	it('applies batch priority multiplier (0.5x)', async () => {
		const usage = createResponseUsage({ input_tokens: 1_000_000, output_tokens: 1_000_000 });
		const cost = await OpenAiCostCalculator.calculateCost('gpt-4o-mini', usage, 'batch');
	...
	it('applies priority multiplier (2x)', async () => {
		const usage = createResponseUsage({ input_tokens: 1_000_000, output_tokens: 1_000_000 });
		const cost = await OpenAiCostCalculator.calculateCost('gpt-4o-mini', usage, 'priority');

		// gpt-4o-mini pricing: input=0.15, output=0.6 -> priority: input=0.3, output=1.2
		// NOTE: due to mutation bug in calculateCost, the pricing may be mutated by prior test.
...
describe('OpenAiCostTrackerSqlite.recordsToCsv', () => {
	...
	it('returns CSV with header and data rows', () => {
		...
		const csv = OpenAiCostTrackerSqlite.recordsToCsv(records);
	...
	it('returns header-only for empty array', () => {
		const csv = OpenAiCostTrackerSqlite.recordsToCsv([]);
