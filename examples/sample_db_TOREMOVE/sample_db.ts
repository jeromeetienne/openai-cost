...
import { OpenAICallTrackerCallback } from "../../src/openai_call_tracker";
...
export type OpenAICostSampleEntry = {
	totalCost: number,
}

export type OpenAICostSampleStorage = {
	spent: Record<string, OpenAICostSampleEntry>;
	saved: Record<string, OpenAICostSampleEntry>;
}
 ...
 */
export class OpenAICostSampleDb {
	private _sampleStorage: OpenAICostSampleStorage = {
		spent: {},
		saved: {},
	}
	...
	/**
	 * Constructor for the OpenAICostSampleDb class.
	...
	async init() {
		...
		if (this._filePath !== undefined) {
			...
			if (fileExists) {
				...
				this._sampleStorage = JSON.parse(fileContent) as OpenAICostSampleStorage;
			}
		}
	}
	...
	getStorage(): OpenAICostSampleStorage {
		return this._sampleStorage;
	}

	getTrackedCallback(): OpenAICallTrackerCallback {
		return this._trackedCallback.bind(this);
	}
	 ...
	 */
	private async _trackedCallback(bucketId: string, response: Response) {
