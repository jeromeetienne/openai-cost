...
async function main() {
	...
	const trackerJson = new OpenAICostTrackerJson(dbFilePath);
	await trackerJson.init();

	// on process exit, we want to save the tracked db to the file
	const onExit = async () => {
		await trackerJson.close();
	}
	...
	const fetchWithTracking = await OpenAICallTracker.getFetchFn(await trackerJson.getTrackerCallback(), {
		bucketId: `bucket-openai-cost-tracker-json-example`,
	...
	console.log(`tracked storage after nostream calls:`, Chalk.cyan(JSON.stringify(await trackerJson.getStorage(), null, 2)));
