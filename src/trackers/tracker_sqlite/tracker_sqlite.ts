...
import { ProviderDetector, ProviderId } from "../../provider-detector";
import {
...
/**
 * SQLite-backed cost tracker for OpenAI-compatible API calls.
 * Stores costs in a SQLite database with schema: dateIso, bucketId, provider, modelName, costSpent, costSaved
 */
export class OpenAiCostTrackerSqlite extends EventEmitter {
	...
	async init(): Promise<void> {
		...
		// Migrate older DBs that pre-date the `provider` column. Older rows can only have come from OpenAI,
		// so the default is correct for historical data.
		const columns = this._database.prepare("PRAGMA table_info(cost_tracking)").all() as Array<{ name: string }>;
		const hasProviderColumn = columns.some(col => col.name === 'provider');
		if (hasProviderColumn === false) {
			this._database.exec("ALTER TABLE cost_tracking ADD COLUMN provider TEXT NOT NULL DEFAULT 'openai'");
		}
	}
	...
	async getAllRecords(): Promise<OpenAiCostTrackerSqliteEntry[]> {
		const stmt = this._database.prepare(
			"SELECT dateIso, bucketId, provider, modelName, costSpent, costSaved FROM cost_tracking ORDER BY dateIso DESC"
		);
		return stmt.all() as OpenAiCostTrackerSqliteEntry[];
	}

	/**
	 * Get a JSON summary of costs grouped by bucket and (provider, model) using SQL aggregation
	 */
	async getSummaryCosts(): Promise<OpenAiCostTrackerSqliteCostSummary> {
		...
		// Get breakdown by bucket, provider and model using SQL grouping
		const breakdownStmt = this._database.prepare(
			"SELECT bucketId, provider, modelName, SUM(costSpent) as costSpent, SUM(costSaved) as costSaved FROM cost_tracking GROUP BY bucketId, provider, modelName ORDER BY bucketId, provider, modelName"
		);
		const breakdownRows = breakdownStmt.all() as Array<{ bucketId: string; provider: ProviderId; modelName: string; costSpent: number; costSaved: number }>;
	...
	/**
	 * Callback to track API costs and store in SQLite. Detects the provider from the request URL.
	 */
	private async _trackerCallback(bucketId: string,
		...
		const provider = ProviderDetector.fromUrl(inputUrl);
		...
		if (contentType !== null) {
			...
			if (contentType.includes("text/event-stream")) {
				if (isChatCompletionsEndpoint) {
					await this._trackerCallback_ChatCompletions_EventStream(bucketId, provider, response);
				} else {
					await this._trackerCallback_Responses_EventStream(bucketId, provider, response);
				}
				return
			}
			...
			if (contentType.includes("application/json") && isEmbeddingsEndpoint === false) {
				if (isChatCompletionsEndpoint) {
					await this._trackerCallback_ChatCompletions_ApplicationJson(bucketId, provider, response);
				} else {
					await this._trackerCallback_Responses_ApplicationJson(bucketId, provider, response);
				}
				return
			}
			...
			if (contentType.includes("application/json") && isEmbeddingsEndpoint === true) {
				await this._trackerCallback_Embeddings(bucketId, provider, response);
				return;
			}
		}
	...
	private async _trackerCallback_Responses_EventStream(bucketId: string, provider: ProviderId, response: Response): Promise<void> {
		...
		while (true) {
			...
			for (const event of eventContents) {
				...
				const costResponse = await OpenAiCostCalculator.calculateLlmCost(modelName, openaiResponseUsage, 'standard', provider);

				// process the usage information (calculate cost and store in SQLite)
				await this._trackerCallbackPostProcess(bucketId, provider, costResponse, modelName, isFromCache);
			}
		}
	}
	 ...
	 */
	private async _trackerCallback_ChatCompletions_EventStream(bucketId: string, provider: ProviderId, response: Response): Promise<void> {
		const contentType = response.headers.get("Content-Type")
		if (contentType === null || contentType.includes("text/event-stream") !== true) {
			throw new Error(`Unexpected content type in response for bucketId ${bucketId}: expected "text/event-stream", got ${contentType}`);
		}
		...
		const costResponse = await OpenAiCostCalculator.calculateLlmCost(lastModelName, responsesUsage, 'standard', provider);
		await this._trackerCallbackPostProcess(bucketId, provider, costResponse, lastModelName, isFromCache);
	}

	/**
	 * Parse a non-streamed chat.completions JSON response and track its cost.
	 */
	private async _trackerCallback_ChatCompletions_ApplicationJson(bucketId: string, provider: ProviderId, response: Response): Promise<void> {
		const contentType = response.headers.get("Content-Type")
		...
		const costResponse = await OpenAiCostCalculator.calculateLlmCost(modelName, responsesUsage, 'standard', provider);
		await this._trackerCallbackPostProcess(bucketId, provider, costResponse, modelName, isFromCache);
	}
	...
	private async _trackerCallback_Responses_ApplicationJson(bucketId: string, provider: ProviderId, response: Response): Promise<void> {
	...
	private async _trackerCallback_Embeddings(bucketId: string, provider: ProviderId, response: Response): Promise<void> {
		// sanity check - ensure content type is application/json, otherwise we may not be able to parse the response correctly
		const contentType = response.headers.get("Content-Type")
		if (contentType === null || contentType.includes("application/json") !== true) {
			throw new Error(`Unexpected content type in response for bucketId ${bucketId}: expected "application/json", got ${contentType}`);
		...
		// Calculate cost from usage information
		const costResponse = await OpenAiCostCalculator.calculateEmbeddingCost(modelName, openaiEmbeddingUsage, provider);
	...
	private async _trackerCallbackPostProcess(
		...
		const insertStmt = this._database.prepare(
			"INSERT INTO cost_tracking (dateIso, bucketId, provider, modelName, costSpent, costSaved) VALUES (?, ?, ?, ?, ?, ?)"
		);
		insertStmt.run(dateIso, bucketId, provider, modelName, costSpent, costSaved);
	}
}
